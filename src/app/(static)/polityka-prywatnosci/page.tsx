...
const pageTitle = "Polityka prywatności – OfferCloud";

const rawPolicyText = `
#### § 1 ADMINISTRATOR DANYCH OSOBOWYCH

1. Administratorem danych osobowych osób korzystających z Serwisu oraz Usług jest Usługodawca, tj. OfferCloud Sp. z o.o. , z siedzibą w Lublinie 20-810, ul. Bukszpanowa 4/3, REGON: 386846048, NIP: 9462699513, KRS: 00008567442. Kontakt z OfferCloud Sp. z o.o. jako administratorem danych osobowych może się odbywać w szczególności mailowo przez adres email [[email protected]](https://offercloud.pl/cdn-cgi/l/email-protection).

#### § 2 SPOSÓB PRZETWARZANIA DANYCH

1. Cel i zakres przetwarzania Twoich danych osobowych określa zakres zgód oraz uzupełnionych danych przesłanych za pomocą odpowiedniego formularza.2. Przetwarzanie danych osobowych Użytkowników może dotyczyć imienia i nazwiska, adresu poczty elektronicznej, numeru telefonu, adresu IP komputera, adresu prowadzonej działalności gospodarczej, numeru rachunku bankowego, jak również innych danych koniecznych w celu wykonywania usług świadczonych zgodnie z Regulaminem.3. Podanie danych osobowych jest dobrowolne, ale brak zgody na przetwarzanie danych osobowych oznaczonych w formularzu jako obligatoryjne uniemożliwi wykonywanie Usług oraz realizację Umów przez Usługodawcę. Charakter Usług świadczonych przez Usługodawcę uniemożliwia ich świadczenie w sposób anonimowy.4. Dane osobowe Użytkowników będą przetwarzane w następujących celach:
...
`.replaceAll(
  '[[email protected]](https://offercloud.pl/cdn-cgi/l/email-protection)',
...
const normalizedPolicyText = rawPolicyText.replace(
  /(\)|\.)\s*(\d+\.)\s+/g,
  "$1\n$2 ",
);

const policySections: PolicySection[] = normalizedPolicyText
  .trim()
  .split(/\n(?=####\s)/)
  .map((chunk) => {
...
export const metadata: Metadata = {
  title: pageTitle,
  description:
    "Polityka prywatności OfferCloud – zasady przetwarzania danych osobowych w ramach Serwisu i Usług.",
};

function renderOrderedList(lines: string[], startIndex: number, nested: boolean) {
  ...
  while (i < lines.length) {
    ...
    const match = orderedItemRegex.exec(line);
    if (!match) break;

    const [, _number, itemTextRaw] = match;
    const itemText = itemTextRaw.trim();

    // If this item looks like an introduction (ends with ":"), try to nest the next ordered list.
    const looksLikeListIntroduction = itemText.endsWith(":");
    let nestedList: React.ReactNode | null = null;

    if (looksLikeListIntroduction) {
      const nestedStart = i + 1;
      let k = nestedStart;
      while (k < lines.length && lines[k].trim().length === 0) k += 1;

      const nextLine = k < lines.length ? lines[k].trim() : "";
      const nextMatch = orderedItemRegex.test(nextLine);

      if (nextMatch) {
        const nestedParsed = renderOrderedList(lines, nestedStart, true);
        nestedList = nestedParsed.node;
    ...
    items.push(
      <li key={`${nested ? "n" : "m"}-${i}-${itemText.slice(0, 20)}`} className="list-decimal">
        {looksLikeListIntroduction ? itemText.slice(0, -1) : itemText}
        {nestedList}
      </li>,
    );
  }

  const node = <ol className={wrapperClassName}>{items}</ol>;
...
function renderPolicyBody(body: string) {
  ...
  // Split by blank lines, but render each ordered block as an <ol>.
  ...
  while (i < lines.length) {
    ...
    if (orderedItemRegex.test(lines[i].trim())) {
      const parsed = renderOrderedList(lines, i, false);
      blocks.push(parsed.node);
      i = parsed.nextIndex;
      continue;
    }

    // Fallback: paragraph line (should be rare with current input).
    const paragraphLines: string[] = [];
    while (i < lines.length && lines[i].trim().length > 0 && !orderedItemRegex.test(lines[i].trim())) {
      paragraphLines.push(lines[i].trim());
      i += 1;
    }
    blocks.push(<p key={`p-${i}`} className="leading-7">{paragraphLines.join(" ")}</p>);
  }
        ...
        <NoCopy className="space-y-10 text-zinc-800 dark:text-zinc-200">
          {policySections.map((section) => (
            <div key={section.title} className="space-y-3">
              ...
              <div className="text-sm leading-7 text-zinc-600 dark:text-zinc-300 sm:text-base sm:leading-8">
                {renderPolicyBody(section.body)}
              </div>
            </div>
          ))}
        </NoCopy>
